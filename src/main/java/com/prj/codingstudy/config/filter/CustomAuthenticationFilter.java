package com.prj.codingstudy.config.filter;
    ...
    private UsernamePasswordAuthenticationToken getAuthRequest(HttpServletRequest request) throws Exception {
        try {
            ...
            UserDto user = UserDto.builder()
                    .mbrEmail(request.getParameter("mbrEmail"))
                    .mbrPw(request.getParameter("mbrPw"))
                    .build();
            log.info("1.CustomAuthenticationFilter :: userId:" + user.getMbrEmail() + " userPw:" + user.getMbrPw());

            // ID와 암호화된 패스워드를 기반으로 토큰 발급
            return new UsernamePasswordAuthenticationToken(user.getMbrEmail(), user.getMbrPw());
